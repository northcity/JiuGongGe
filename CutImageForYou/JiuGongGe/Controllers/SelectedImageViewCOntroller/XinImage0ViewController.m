...
@interface XinImage0ViewController ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
- (void)presentXin8ImageViewController{
    //初始化UIImagePickerController类
    self.picker8 = [[UIImagePickerController alloc] init];
    //判断数据来源为相册
    self.picker8.sourceType = UIImagePickerControllerSourceTypePhotoLibrary;
    self.picker8.allowsEditing = YES;
    
    
    //设置代理
    self.picker8.delegate = self;
    //打开相册
    [self presentViewController:self.picker8 animated:YES completion:nil];
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    if ([picker isEqual:self.picker8]) {
        UIImage *image = info[UIImagePickerControllerEditedImage];
        [self.imageViewArray addObject:image];
